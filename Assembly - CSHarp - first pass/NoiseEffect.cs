...
public class NoiseEffect : MonoBehaviour
{
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		Vector4 vector = new Vector4(Random.value, Random.value, (float)Screen.width / (float)grainTexture.width * num, (float)Screen.height / (float)grainTexture.height * num);
		material.SetVector("_GrainOffsetScale", vector);
		Vector4 vector2 = new Vector4(scratchX + Random.value * scratchJitter, scratchY + Random.value * scratchJitter, (float)Screen.width / (float)scratchTexture.width, (float)Screen.height / (float)scratchTexture.height);
		material.SetVector("_ScratchOffsetScale", vector2);
		Vector4 vector3 = new Vector4(Random.Range(grainIntensityMin, grainIntensityMax), Random.Range(scratchIntensityMin, scratchIntensityMax), 0f, 0f);
		material.SetVector("_Intensity", vector3);
