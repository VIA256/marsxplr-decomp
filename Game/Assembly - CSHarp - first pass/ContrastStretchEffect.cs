...
public class ContrastStretchEffect : MonoBehaviour
{
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		ImageEffects.BlitWithMaterial(materialLum, source, renderTexture);
		while (renderTexture.width > 1 || renderTexture.height > 1)
		{
			...
			ImageEffects.BlitWithMaterial(materialReduce, renderTexture, temporary);
	...
	private void CalculateAdaptation(Texture curTexture)
	{
		...
		Material material = materialAdapt;
		Vector4 vector = new Vector4(value, limitMinimum, limitMaximum, 0f);
		material.SetVector("_AdaptParams", vector);
		ImageEffects.BlitWithMaterial(materialAdapt, adaptRenderTex[num], adaptRenderTex[curAdaptIndex]);
	}
}
