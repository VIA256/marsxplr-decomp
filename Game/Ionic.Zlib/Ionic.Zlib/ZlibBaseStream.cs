		...
		private void finish()
		{
			...
			else
			{
				...
				if (_z.TotalBytesOut != 0)
				{
					...
					{
						throw new ZlibException(string.Format("Protocol error. AvailableBytesIn={0}, expected 8", _z.AvailableBytesIn));
					}
					...
					{
						throw new ZlibException(string.Format("Bad CRC32 in GZIP stream. (actual({0:X8})!=expected({1:X8}))", crc32Result2, num2));
					}
					if (num4 != num3)
					{
						throw new ZlibException(string.Format("Bad size in GZIP stream. (actual({0})!=expected({1}))", num4, num3));
					}
				}
			}
		}
