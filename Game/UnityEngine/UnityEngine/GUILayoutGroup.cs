...
namespace UnityEngine
{
	public class GUILayoutGroup : GUILayoutEntry
	{
		...
		public override void CalcWidth()
		{
			if (entries.Count == 0)
			{
				int horizontal = base.style.padding.horizontal;
				maxWidth = (minWidth = horizontal);
				return;
			}
			childMinWidth = 0f;
			childMaxWidth = 0f;
		...
		public override void CalcHeight()
		{
			if (entries.Count == 0)
			{
				int vertical = base.style.padding.vertical;
				maxHeight = (minHeight = vertical);
				return;
			}
			...
			if (isVertical)
			{
				...
				foreach (GUILayoutEntry entry in entries)
				{
					...
					if (entry.style != GUILayoutUtility.spaceStyle)
					{
						...
						if (!flag)
						{
							num4 = ((num3 <= rectOffset.top) ? rectOffset.top : num3);
						}
						else
						{
							num4 = 0;
		...
		public override string ToString()
		{
			...
			text = string.Concat(text, base.ToString() + " Margins: " + childMinHeight + " {\n");
			GUILayoutEntry.indent += 4;
			foreach (GUILayoutEntry entry in entries)
			{
				text = string.Concat(text, entry.ToString() + "\n");
			}
