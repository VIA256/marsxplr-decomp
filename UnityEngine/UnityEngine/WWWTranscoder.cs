...
namespace UnityEngine
{
	public class WWWTranscoder
	{
		...
		{
			return Encoding.ASCII.GetString(Encode(e.GetBytes(toEncode), urlEscapeChar, urlSpace, urlForbidden, false));
		}
		...
		{
			return Encoding.ASCII.GetString(Encode(e.GetBytes(toEncode), qpEscapeChar, qpSpace, qpForbidden, true));
		}
			...
			using (MemoryStream memoryStream = new MemoryStream(input.Length * 2))
			{
				try
				{
					for (int i = 0; i < input.Length; i++)
					{
						...
						else if (input[i] < 32 || input[i] > 126 || Array.IndexOf(forbidden, input[i]) != -1)
						{
