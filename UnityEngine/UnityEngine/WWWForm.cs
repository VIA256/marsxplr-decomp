			...
			get
			{
					...
					using (MemoryStream memoryStream = new MemoryStream(1024))
					{
						try
						{
							for (int i = 0; i < formData.Count; i++)
							{
								...
								memoryStream.Write(bytes2, 0, bytes2.Length);
								memoryStream.Write(bytes2, 0, bytes2.Length);
								memoryStream.Write(formData[i], 0, formData[i].Length);
							}
							memoryStream.Write(bytes2, 0, bytes2.Length);
							memoryStream.Write(bytes, 0, bytes.Length);
							memoryStream.Write(boundary, 0, boundary.Length);
				...
				using (MemoryStream memoryStream2 = new MemoryStream(1024))
				{
					try
					{
						...
						{
							byte[] array = WWWTranscoder.URLEncode(Encoding.UTF8.GetBytes(fieldNames[j]));
							byte[] array2 = WWWTranscoder.URLEncode(formData[j]);
							if (j > 0)
							{
