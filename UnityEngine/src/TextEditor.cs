...
namespace UnityEngine
{
	public class TextEditor : IKeyboardControl
	{
		...
		{
			char[] val = new char[1] { c };
			ReplaceSelection(new string(val));
		}
		...
		public void DrawCursor(string text)
		{
			...
			graphicalCursorPos = gUIStyle.GetCursorPixelPosition(rect, content, pos);
			Rect rect2 = style.padding.Remove(position);
			Vector2 vector = style.CalcSize(content);
			if (vector.x < rect2.width)
			{
				scrollOffset.x = 0f;
			}
			else
			{
				if (graphicalCursorPos.x < scrollOffset.x)
				{
					scrollOffset.x = graphicalCursorPos.x;
				}
				else if (graphicalCursorPos.x > scrollOffset.x + rect2.width)
				{
					scrollOffset.x = graphicalCursorPos.x - rect2.width + 1f;
				}
				if (scrollOffset.x + rect2.width > vector.x)
				{
					scrollOffset.x = vector.x - rect2.width + 1f;
				}
			}
			if (vector.y < rect2.height)
			{
				scrollOffset.y = 0f;
			}
			else
			{
				if (graphicalCursorPos.y < scrollOffset.y)
				{
					scrollOffset.y = graphicalCursorPos.y + 1f;
				}
				else if ((double)graphicalCursorPos.y >= (double)(scrollOffset.y + rect2.height) - 3.0)
				{
					scrollOffset.y = graphicalCursorPos.y + 1f;
				}
				if (scrollOffset.y + rect2.height > vector.y)
				{
					scrollOffset.y = vector.y - rect2.height + 1f;
				}
			}
