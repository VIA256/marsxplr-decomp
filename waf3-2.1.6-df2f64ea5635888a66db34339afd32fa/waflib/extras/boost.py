...
def options(opt):
	...
	opt.add_option('--boost-includes',type=str,default='',dest='boost_includes',help='''path to the directory where the boost includes are,
				   e.g., /path/to/boost_1_55_0/stage/include''')
	opt.add_option('--boost-libs',type=str,default='',dest='boost_libs',help='''path to the directory where the boost libs are,
	...
	opt.add_option('--boost-abi',type=str,default='',dest='boost_abi',help='''select libraries with tags (gd for debug, static is automatically added),
	...
	opt.add_option('--boost-toolset',type=str,default='',dest='boost_toolset',help='force a toolset e.g. msvc, vc90, \
						gcc, mingw, mgw45 (default: auto)')
	py_version='%d%d'%(sys.version_info[0],sys.version_info[1])
	opt.add_option('--boost-python',type=str,default=py_version,dest='boost_python',help='select the lib python with this version \
...
def boost_get_libs(self,*k,**kw):
	...
	def match_libs(lib_names,is_static):
		...
		ext=ext.partition('%s')[2]
		for lib in lib_names:
			if lib=='python':
				tags='({0})?((-py{2})|(-py{1}(?=[^0-9]))|({2})|(-{1}.{3})|({1}(?=[^0-9]))|(?=[^0-9])(?!-py))'.format(tags_pat,kw['python'][0],kw['python'],kw['python'][1])
			else:
				tags=tags_pat
			for pattern in['boost_%s%s%s%s%s$'%(lib,toolset_pat,tags,version,ext),'boost_%s%s%s%s$'%(lib,tags,version,ext),'boost_%s%s%s%s$'%(lib,toolset_pat,tags,ext),'boost_%s%s%s$'%(lib,tags,ext),'boost_%s%s$'%(lib,ext),'boost_%s'%lib]:
...
def _check_pthread_flag(self,*k,**kw):
	...
	if Utils.unversioned_sys_platform()=="sunos":
		...
		boost_pthread_flags=["","-lpthreads","-Kthread","-kthread","-llthread","-pthread","-pthreads","-mthreads","-lpthread","--thread-safe","-mt"]
...
def check_boost(self,*k,**kw):
	...
	self.end_msg("%d.%d.%d"%(int(versions[1])/100000,int(versions[1])/100%1000,int(versions[1])%100))
	if Logs.verbose:
